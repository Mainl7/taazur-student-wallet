...
توسعة المسارات المتفق عليها: `/schools`، `/students`، `/cards`، `/wallets/top-up`، `/reports`، `/audit-logs`. جميعها تحت `/api/v1` وتحتاج مخطط Zod، صلاحية، ونطاق مدرسة عند إضافتها. مسار `/wallets/top-up` يمثل تخصيص مبلغ فسحة من الجمعية للطالب.
