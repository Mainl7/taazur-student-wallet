...
export default function CanteenUsers() {
  ...
  const load = async () => {
    const [schoolResponse, userResponse] = await Promise.all([apiFetch('/schools'), apiFetch('/canteen-users')]);
    if (schoolResponse.status === 401 || userResponse.status === 401) return location.assign('/login');
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    form.reset();
    setMessage('تم إنشاء حساب المقصف وربطه بالمدرسة.');
    void load();
  }

  return (
    <AdminShell>
      <header><div><h1>حسابات المقصف</h1><a href="/canteen">فتح صفحة المقصف ←</a></div></header>
      ...
      <table>
        <thead><tr><th>البريد</th><th>المدرسة</th><th>رمز المدرسة</th><th>تاريخ الإنشاء</th></tr></thead>
        <tbody>{users.map(user => <tr key={user.id}><td>{user.email}</td><td>{user.school?.name ?? '—'}</td><td>{user.school?.schoolCode ?? '—'}</td><td>{new Date(user.createdAt).toLocaleString('ar-SA')}</td></tr>)}</tbody>
      </table>
    </AdminShell>
  );
}
