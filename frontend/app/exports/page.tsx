  ...
  return (
    <AdminShell>
      ...
      <div className="export-grid">
        <a className="export-card" href={`${api}/exports/monthly-expenses.xls?${query}`}><strong>تقرير شهري Excel</strong><span>مصروفات الطلاب جاهزة للفتح في Excel والطباعة.</span></a>
        <a className="export-card" href={`${api}/exports/monthly-expenses-print?${query}`} target="_blank"><strong>تقرير شهري PDF</strong><span>يفتح تقرير طباعة؛ اختر طباعة ثم حفظ كـ PDF.</span></a>
        <a className="export-card" href={`${api}/exports/transactions.csv?${query}`}><strong>تصدير العمليات</strong><span>كل عمليات الشهر المحدد بصيغة CSV.</span></a>
        <a className="export-card" href={`${api}/exports/students.csv${schoolQuery}`}><strong>تصدير الطلاب</strong><span>بيانات الطلاب والرصيد ورمز البطاقة النشطة.</span></a>
      </div>
    </AdminShell>
  );
}
